...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { results } = await db.prepare(`
      ...
      LIMIT 50
    `).all<{
      user_id: string; total_score: number; rank: string;
      song_title: string; song_artist: string; youtube_id: string;
    ...
    if (isMissingDBBinding(error)) return NextResponse.json({ ranking: [] });
    throw error;
  }
}
